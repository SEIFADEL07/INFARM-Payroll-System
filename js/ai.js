...
const AIAssistant = {
    ...
    processOfflineQuery: function(query, employees) {
        ...
        if (cleanQuery.includes('أعلى راتب') || cleanQuery.includes('أعلى موظف') || cleanQuery.includes('صاحب أعلى راتب') || cleanQuery.includes('highest salary') || cleanQuery.includes('highest paid')) {
            ...
            const top = sorted[0];
            return {
                text: `الموظف صاحب أعلى صافي راتب هو <strong>${HtmlSafety.escape(top.name)}</strong> من قسم <strong>${HtmlSafety.escape(top.department)}</strong>.<br>
                       <ul>
        ...
        if (cleanQuery.includes('أقل راتب') || cleanQuery.includes('أدنى راتب') || cleanQuery.includes('أقل موظف') || cleanQuery.includes('lowest salary') || cleanQuery.includes('lowest paid')) {
            ...
            return {
                text: `الموظف صاحب أقل صافي راتب هو <strong>${HtmlSafety.escape(bottom.name)}</strong> من قسم <strong>${HtmlSafety.escape(bottom.department)}</strong>.<br>
                       <ul>
        ...
        if (cleanQuery.includes('أخطاء') || cleanQuery.includes('تحذير') || cleanQuery.includes('مشاكل') || cleanQuery.includes('غير مطابق') || cleanQuery.includes('errors') || cleanQuery.includes('warnings')) {
            ...
            issues.forEach(emp => {
                ...
                rows += `<tr>
                            <td>${HtmlSafety.escape(emp.name)}</td>
                            <td>${HtmlSafety.escape(emp.department)}</td>
                            <td>${emp.calculatedNet.toFixed(2)}</td>
                            <td style="color: ${emp.validationStatus === 'error' ? '#ef4444' : '#f59e0b'}">${HtmlSafety.escape(alertsList)}</td>
                         </tr>`;
            });
        ...
        if (cleanQuery.includes('أقسام') || cleanQuery.includes('ادارات') || cleanQuery.includes('الأقسام والإدارات') || cleanQuery.includes('عدد الاقسام')) {
            ...
            depts.forEach(d => {
                ...
                deptDetails += `<li><strong>${HtmlSafety.escape(d)}:</strong> ${deptEmps.length} موظفاً (إجمالي الصافي: ${deptNet.toLocaleString('ar-SA')} SAR)</li>`;
            });
            return {
        ...
        if (matchedDept) {
            ...
            return {
                text: `إليك تفاصيل الرواتب لقسم <strong>${HtmlSafety.escape(matchedDept.toUpperCase())}</strong>:<br>
                       <ul>
        ...
        let foundEmp = null;
        const normQuery = ExcelParser.normalizeArabicName(query);
        if (normQuery) {
            for (const emp of employees) {
                const normName = ExcelParser.normalizeArabicName(emp.name);
        ...
        if (foundEmp) {
            return {
                text: `بيانات راتب الموظف <strong>${HtmlSafety.escape(foundEmp.name)}</strong> (${HtmlSafety.escape(foundEmp.department)}):<br>
                       <ul>
