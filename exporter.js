        ...
        tempDiv.innerHTML = `
            <div style="border: 2px solid #10b981; border-radius: 16px; padding: 24px; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; padding: 16px; margin-bottom: 24px; font-size: 0.9rem;">
                    ...
                    <div><strong>القسم / الإدارة:</strong> ${employee.department}</div>
                    <div><strong>الرقم المرجعي:</strong> ${employee.employeeId || '#EMP' + String(employee.id).padStart(4, '0')}</div>
                ...
                ${employee.validationAlerts.length > 0 ? `
                    <div style="background: #fdf2f2; border: 1px solid #fde8e8; border-radius: 8px; padding: 12px; margin-bottom: 24px; font-size: 0.85rem; color: #9b1c1c;">
                        ...
                        <ul style="margin: 6px 18px 0 0; padding: 0;">
                            ${employee.validationAlerts.map(a => `<li>${a.message}</li>`).join('')}
                        </ul>
                    </div>
                ` : ''}
        ...
        employees.forEach((emp, index) => {
            tableRows += `
                <tr style="${index % 2 === 0 ? 'background: #f8fafc;' : ''}">
                    <td style="padding: 8px 10px; border: 1px solid #cbd5e1; text-align: center;">${emp.employeeId || index + 1}</td>
                    <td style="padding: 8px 10px; border: 1px solid #cbd5e1;">${emp.name}</td>
                    <td style="padding: 8px 10px; border: 1px solid #cbd5e1; text-align: center;">${emp.department}</td>
        ...
        tempDiv.innerHTML = `
            <div style="border: 1px solid #cbd5e1; border-radius: 12px; padding: 20px;">
                ...
                <div style="text-align: center; margin-bottom: 20px;">
                    <h2 style="margin: 0; font-size: 1.2rem; color: #475569; font-weight: 700;">تقرير مسيرات الرواتب الإجمالي (المجموعة: ${sheetName})</h2>
